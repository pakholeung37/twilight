...
import { KonvaNodeComponent } from "@twilight/react-konva"
...
>(
  ...
  PropsWithoutRef<KonvaProps> & RefAttributes<KonvaNode>
> {
  function PortalContainer(
    props: PropsWithChildren<KonvaProps>,
    ref: ForwardedRef<KonvaNode>,
  ) {
    ...
    const Konva: any = Component as React.ComponentType<KonvaProps>

    useImperativeHandle<KonvaNode | null, KonvaNode | null>(
      ref,
      () => konvaRef.current,
      [konvaRef.current],
    )
    ...
    useLayoutEffect(() => {
      if (id != null && managerRef.current)
        stage?.addManager(id, managerRef.current)
      return () => {
