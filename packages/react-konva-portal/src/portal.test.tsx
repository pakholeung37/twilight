...
import { Stage as ReactKonvaStage, Text } from "@twilight/react-konva"
...
describe("portal.tsx", () => {
  describe("content movement", () => {
    ...
    function findFirst<Node extends Konva.Node>(
      stage?: Konva.Stage,
      query?: string,
    ) {
    ...
    function assertNotFound(stage?: Konva.Stage, ...ids: string[]) {
      for (const id of ids) {
        ...
        const textNodes = container?.children
          .toArray()
  ...
  describe("content ordering", () => {
    class App extends Component<
      { a: PortalProps; b: PortalProps },
      { a: PortalProps; b: PortalProps }
    > {
      public state = { a: this.props.a, b: this.props.b }
      public stageRef: ReactKonvaStage | null = null

      public moveA = (to: PortalProps) =>
        this.setState(from => ({ ...from, a: { ...from.a, ...to } }))
      public moveB = (to: PortalProps) =>
        this.setState(from => ({ ...from, b: { ...from.b, ...to } }))
    ...
    function assertOrder(stage: Konva.Stage, id: string, ...ordered: string[]) {
      ...
      ordered.forEach((content, index) =>
        expect(children[index].name()).toEqual(content),
      )
    }
    ...
    it("when mounted in specific order", () => {
      const wrapper = mount<App>(
        <App a={portalProps(555)} b={portalProps(111)} />,
      )
