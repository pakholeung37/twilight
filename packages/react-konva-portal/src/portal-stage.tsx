...
import { Stage } from "@twilight/react-konva"
...
function StageComponent(
  { children, portalLayerProps, ...props }: PortalStageProps,
  ref: ForwardedRef<typeof Stage>,
) {
  ...
  useImperativeHandle<typeof Stage | null, typeof Stage | null>(
    ref,
    () => stageRef.current,
    [stageRef.current],
  )
  ...
  useLayoutEffect(() => {
    Object.keys(queueRef.current).forEach(id =>
      managersRef.current[id]?.handle(queueRef.current[id]),
    )
  ...
  const mount = useCallback(
    (id: string, zIndex: number, children: ReactNode) => {
      const key = ++seqRef.current
      const managerReady = managersRef.current[id]
      if (managerReady && !delayRef.current)
        managerReady.handle(mountCmd(key, zIndex, children))
      else {
  ...
  const update = useCallback(
    (id: string, key: number, zIndex: number, children: ReactNode) => {
      const managerReady = managersRef.current[id]
      if (managerReady && !delayRef.current)
        managerReady.handle(updateCmd(key, zIndex, children))
      else {
        const queue = queueRef.current[id] || []
  ...
  return (
    <Stage ref={stageRef as any} {...props}>
      <Provider
        mount={mount}
        update={update}
        unmount={unmount}
...
const PortalStage = forwardRef<typeof Stage, PortalStageProps>(StageComponent)
