...
function ManagerComponent(
  ...
  ref: ForwardedRef<PortalManagerRef>,
) {
    ...
    () => {
      ...
      function unmount(cmd: UnmountCommand) {
        ...
        portalsRef.current = portalsRef.current.filter(
          item => item.key !== cmd.key,
        )
      }

      const handlers = { mount, unmount, update } as Record<
        string,
        CommandHandler
      >
...
const PortalManager = forwardRef<PortalManagerRef, PortalManagerProps>(
  ManagerComponent,
)
